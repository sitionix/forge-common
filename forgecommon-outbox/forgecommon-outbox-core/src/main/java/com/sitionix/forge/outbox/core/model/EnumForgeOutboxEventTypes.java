...
/**
 * Enum-backed implementation of {@link ForgeOutboxEventTypes}.
 */
public class EnumForgeOutboxEventTypes<E extends Enum<E> & ForgeOutboxEventType> implements ForgeOutboxEventTypes {

    private final Map<String, E> byDescription;

    public EnumForgeOutboxEventTypes(final Class<E> enumClass) {
    ...
    @Override
    public ForgeOutboxEventType byDescription(final String description) {
        ...
        if (eventType == null) {
            throw new IllegalArgumentException("No outbox event type found for description: " + normalized);
        }
        return eventType;
    }
